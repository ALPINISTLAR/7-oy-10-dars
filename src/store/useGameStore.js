...
import getRandomPhrase from "../general/utils/getRandomPhrase";
const categories = {
  Movies: [
    ...
    { name: "Schindlers List", selected: false },
...
const useGameStore = create(
  devtools(
    immer(set => ({
      ...
      setSelected: () =>
        set(state => {
          state.categories[state.category].find(
            catWords => catWords.name === state.phrase
          ).selected = true;
        }),
      ...
      setCategory: category =>
        set(state => {
          ...
          state.phrase = getRandomPhrase(state.categories[category]);
        }),

      setPressedKey: key =>
        set(state => {
          state.pressedKeys.push(key);
          const lowerPhrase = state.phrase.toLowerCase().replaceAll(" ", "");

          if (
            lowerPhrase
              .split("")
              .every(letter => state.pressedKeys.includes(letter))
          ) {
          ...
          if (state.gameStatus === "playing" && !lowerPhrase.includes(key)) {
            state.health--;
          }
      ...
      playAgain: () =>
        set(state => {
          ...
          state.pressedKeys = [];
          state.gameStatus = "playing";
          state.phrase = getRandomPhrase(state.categories[state.category]);
